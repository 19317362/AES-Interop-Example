...
var slowAES = {
    ...
    encrypt: function (bytesIn, mode, pad, key, iv) {
        ...
        var firstRound = true;
        if (bytesIn !== null) {
            var stop = Math.ceil((bytesIn.length)/16); // ceil ,not need +1
...
function getKeyAndIv(password, salt, iterations, callback) {
  ...
  var ksz = getRadioValue('keysize');
  showMsg('keysize (bits): ' + ksz);      // in bits
  var keysize = parseInt(ksz) / 8; // in bytes
...
function decrypt_click() {
  ...
  key = e.value;
    ...
    var doTheWork = function(key,iv) {
        ...
        var ciphertext = hexStringToByteArray(e1.value);

        var decrypted = slowAES.decrypt(ciphertext,
                                        mode,
                                        padding,
                                        key,
                                        iv);

        var plaintext = byteArrayToString(decrypted);
